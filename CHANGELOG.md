...
Added "Export to Folder…": right-click one or more files/folders in the Explorer and copy them out to a folder you pick via a dialog. Folders are copied recursively (including empty subdirectories); the same Overwrite/Skip collision prompt as Import applies to any file that already exists at the destination.